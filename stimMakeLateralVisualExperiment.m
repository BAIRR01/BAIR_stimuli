function stimMakeLateralVisualExperiment(stimParams, runNumber, stimulusType, onsetTimeMultiple, TR)
...
imageSizeInPixels = round(6.5/20 * size(stimParams.stimulus.images));
...
switch stimulusType
    case 'SCENEFACELATERAL'
        ...
        fname    = 'scenefacelateral.mat';
        load(fullfile(stimDir, fname));

        fprintf('[%s]: Loading stimulus indices...\n',mfilename);
        fname    = 'stimIDs_scenefacelateral.mat';
        load(fullfile(stimDir, fname));
        ...
        for cc = 1:numberOfCategories
            ...
            % Different images for different runs
            if mod(runNumber,2) ~= 0
                iminx = 1:2:size(stimIDs,3);
            else
                iminx = 2:2:size(stimIDs,3);
            end
            imageIndex = stimIDs(cc, 1, iminx);
        ...
        % Now, duplicate images creating one on left and one on right
        newimages = [];
        newcatindex = [];
        count = 1;
        visfieldloc = [];
        for ii = 1:size(images,4)
            inputImage = images(:,:,:,ii);


            pixelsToAdd_horizontal = size(stimParams.stimulus.images,1)-imageSizeInPixels(1);
            pixelsToAdd_vertical = (size(stimParams.stimulus.images,1)-imageSizeInPixels(1))/2;
            
            % RIGHT VF

            % pad with background color along width dimension

            dim = 2; % horizontal
            padImage = ones([imageSizeInPixels(1) pixelsToAdd_horizontal 3])*128; % backgroundColor
            paddedImage = cat(dim, padImage, inputImage);
            
            % pad with background color along height dimension
            dim = 1; % vertical
            padImage = ones([pixelsToAdd_vertical size(stimParams.stimulus.images,2) 3])*128; % backgroundColor
            paddedImage = cat(dim, padImage, paddedImage, padImage);

            newimages(:,:,:,count) = paddedImage;
            visfieldloc(count) = 1;
            newcatindex(count) = catindex(ii);
            count = count + 1;
            ...
            dim = 2; % horizontal
            padImage = ones([imageSizeInPixels(1) pixelsToAdd_horizontal 3])*128; % backgroundColor
            paddedImage = cat(dim, inputImage, padImage);
        ...
        newimages = uint8(newimages);
        images = newimages;
        catindex = newcatindex;
        
        % Set durations and ISI
        durations = ones(1,size(images,4))*0.3;
...
stimulus.visfieldloc  = visfieldloc;
...
stimulus.fixSeq = fixSeq+2; % black vs white cross
...
visfieldloc = stimulus.visfieldloc(stimulus.trialindex)';

stimulus.tsv = table(onset, duration, ISI, trial_type, trial_name, stim_file, stim_file_index, visfieldloc);
